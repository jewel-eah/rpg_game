...
public class Guild {
	...
	public void setGuild() {
		Unit temp = new Unit("하리보석", 2, 100, 10, 5, 0);
		this.guildList.add(temp);
		temp = new Unit("꿀단지은", 3, 80, 7, 3, 0);
		this.guildList.add(temp);
		temp = new Unit("황도백도하", 8, 50, 3, 1, 0);
		this.guildList.add(temp);
		temp = new Unit("또배고파진경", 1, 70, 5, 2, 0);
		this.guildList.add(temp);
		temp = new Unit("예지금뭐하니", 4, 200, 4, 8, 0);
		this.guildList.add(temp);
		temp = new Unit("세상참말세린", 1, 120, 11, 7, 0);
		this.guildList.add(temp);
		for (int i = 0; i < 4; i++) {
	...
	private void printParty() {
		...
		for (int i = 0; i < PARTY_SIZE; i++) {
			boolean partyStatus = guildList.get(i).isParty();
			System.out.print("[" + (i + 1) + "번]");
	...
	private void partyStatus( boolean guildInfo) {
		if(guildInfo == true) {
			System.out.print("⭕️");
		}
		else {
	...
	private void sortUserByParty() {
		for(int i=0; i<guildList.size(); i++) {
			Unit unit = this.guildList.get(i);
			boolean check = unit.isParty();
			int index = i;
			
			for(int j=i; j<guildList.size(); j++) {
				Unit nextUnit = this.guildList.get(j);
				if(!check && (nextUnit.isParty())) {
					check = nextUnit.isParty();
					index = j;
				}
			}
			Unit temp = guildList.get(i);
	...
	private void sortUserByLevel() {
		for(int i=0; i<guildList.size(); i++) {
			...
			for(int j=i; j<guildList.size(); j++) {
				...
				if((level-nextUnit.getLevel()) < 0) {
					level = nextUnit.getLevel();
					index = j;
				}
			}
